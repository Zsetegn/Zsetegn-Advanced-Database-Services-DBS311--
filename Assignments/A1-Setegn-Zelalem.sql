...
SELECT c.customer_id,
      ...
      tO_CHAR(SUM(QUANTITY*UNIT_PRICE),'$999,999,999,999.99') AS total_amount
FROM   customers c
       JOIN orders o
         ON c.customer_id = o.customer_id
       JOIN order_items oi
...
SELECT c.customer_id,
       ...
       Sum(oi.quantity)                 AS "total items",
           tO_CHAR(Sum(oi.quantity * oi.unit_price),'$999,999,999,999.99') AS "total amount"
FROM   orders o,
       customers c,
       order_items oi
...
SELECT w.warehouse_id,
       ...
       p.category_id,
       category_name,
       tO_CHAR(Min(standard_cost) ,'$999,999,999,999.99')AS lowest_cost
FROM   inventories i
       JOIN warehouses w
...
SELECT To_char(To_date(the_month, 'MM'), 'Month') AS "MONTH",
       counts                                     AS "number OF orders"
FROM   (SELECT Extract(month FROM order_date) AS the_month,
               Count(*)                       AS counts
        FROM   orders
        GROUP  BY Extract(month FROM order_date)) sales
ORDER  BY the_month;
